...
class JavaSinkTracer:
    def __init__(self, project_path: str, rules_path: str, on_partial=None):
        ...
        for root, _, files in os.walk(self.project_path):
            for file in files:
                ...
                try:
                    ...
                    # 心跳：每解析若干文件就触发一次进度写入
                    try:
                        if self.on_partial and (self.stats['parsed_files'] % 10 == 0):
                            # 动态速率
                            elapsed = time.time() - self.stats_start_ts
                            rate = (self.stats['parsed_files'] / elapsed) * 60.0 if elapsed>0 else 0.0
                            self.stats['rate_per_min'] = round(rate, 2)
                    ...
                    print(Fore.RED + f"[错误]文件解析失败：{file}, 原因：{str(e)}")
                    continue

        print(Fore.LIGHTBLUE_EX + f"[+]AST构建全部完成！")
        # 速率统计：文件/分钟
        elapsed = (time.time() - self.stats_start_ts) if hasattr(self, 'stats_start_ts') else 0.001
        rate = (self.stats['parsed_files'] / elapsed) * 60.0 if elapsed>0 else 0.0
        self.stats['rate_per_min'] = round(rate, 2)
        print(Fore.CYAN + f"[统计]总文件: {self.stats['total_files']}, 已解析: {self.stats['parsed_files']}, 已跳过: {self.stats['skipped_files']}, 错误: {self.stats['error_files']}, 速率: {self.stats['rate_per_min']} 文件/分钟")
            ...
            for sink in rule["sinks"]:
                ...
                for method in methods.split("|"):
                    ...
                    if paths:
                        ...
                        results.append(item)
                        # 写入实时进度（轻量模式也心跳），便于前端看到“已找到X条疑似链路”
                        try:
                            self.partial_results.append(item)
                            if self.on_partial:
                                self.on_partial(self.partial_results)
        ...
        while queue:
            ...
            for caller in caller_methods:
                ...
                if caller in path_nodes:
                    # print(Fore.RED + f"[!] 检测到环引用，跳过: {caller}")
                    continue
