...
def generate_markdown_report(project_name, local_project_path, json_file_path, output_file_path):
    ...
    # 读取可选 meta，用于展示筛选条件等附加信息
    meta_path = os.path.join(output_file_path, 'vulns_meta.json')
    meta = {}
    try:
    ...
    with open(markdown_file_path, 'w', encoding='utf-8') as md:
        ...
        # 标注筛选条件
        if meta.get('filters'):
            md.write(f"- 应用筛选：{json.dumps(meta['filters'], ensure_ascii=False)}\n")
        md.write("\n")
        for sink_item in sink_json_data:
            vtype = sink_item.get('vul_type', '未知漏洞类型')
            chains = sink_item.get('call_chains', []) or []
            md.write(f"# {vtype} 漏洞（{len(chains)} 个）\n\n")
            # 基础信息
            md.write("**1）漏洞基础信息**\n\n")
            md.write(f"- 漏洞简述: {sink_item.get('sink_desc', '无')}\n\n")
            md.write(f"- 严重等级: **{sink_item.get('severity', '无')}**\n\n")
            md.write(f"- Sink函数: **{sink_item.get('sink', '无')}**\n\n")
            if sink_item.get('file_path'):
                md.write(f"- 文件: `{sink_item.get('file_path')}`\n\n")
            gl = sink_item.get('group_lines')
            if gl:
                # 展示命中行范围或列表
                try:
                    if isinstance(gl, list):
                        # 合并连续行段为范围
                        gl_sorted = sorted(set(int(x) for x in gl))
                        ranges = []
                        start = prev = gl_sorted[0]
                        for ln in gl_sorted[1:]:
                            if ln == prev + 1:
                                prev = ln
                            else:
                                ...
                                start = prev = ln
                        ranges.append((start, prev))
                        rng_text = ', '.join([f"{a}" if a==b else f"{a}-{b}" for a,b in ranges])
                        md.write(f"- 模板命中行: {rng_text}\n\n")
                except Exception:
                    md.write(f"- 模板命中行: {gl}\n\n")
            # 调用链
            md.write("**2）调用链路信息**\n\n")
            if chains and isinstance(chains[0], dict):
                # 兼容旧结构：每个元素包含 chain/code
                for chain_item in chains:
                    for seg in chain_item.get('chain', []):
                        md.write(f"- {seg}\n")
                    md.write("\n")
                # 代码片段（如有）
                if any(ci.get('code') for ci in chains):
                    md.write("**3）链路完整代码**\n\n")
                    md.write("```java\n")
                    for chain_item in chains:
                        for seg, code_line in zip(chain_item.get('chain', []), chain_item.get('code', [])):
                            md.write(f"// {seg}\n")
                            md.write(f"{code_line}\n\n")
                    md.write("```\n\n")
            else:
                # 新结构：chains 为字符串序列或数组序列
                for chain in chains:
                    if isinstance(chain, list):
                        md.write(f"- {' → '.join(chain)}\n")
                    else:
                        md.write(f"- {chain}\n")
                md.write("\n")
            md.write("\n")
...
<head>
    ...
    <style>
        ...
        .codehilite .gh {{ color: #87d7d7; }}                     /* Generic.Heading */
...
<body>
    ...
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(full_html)
    print(Fore.LIGHTMAGENTA_EX + f"[+]审计结果已保存到：{html_path}")
